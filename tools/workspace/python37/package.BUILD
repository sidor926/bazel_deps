...
genrule(
    name = "move_stdlib",
    srcs = ALL_STDLIB,
    outs = ["lib/python3.7/" + x[4:] for x in ALL_STDLIB],
    cmd = "SRC=($(SRCS)) OUT=($(OUTS)); for (( i=0; i < $${#SRC[*]}; ++i)); do in=$${SRC[$$i]}; out=$${OUT[$$i]}; cp $$in $$out 1>&2; done"
)

[cc_binary(
    name = "lib/python3.7/lib-dynload/" + item + ".cpython-37m-unknown.so",
...
filegroup(
    name = "modules",
    srcs = ["lib/python3.7/lib-dynload/" + item + ".cpython-37m-unknown.so"
...
cc_binary(
    ...
    data = [
        ":modules",
        ":move_stdlib",
        "lib/python3.7/_sysconfigdata_m_unknown_.py",
    ],
)
...
generate_file(
    name = "lib/python3.7/_sysconfigdata_m_unknown_.py",
